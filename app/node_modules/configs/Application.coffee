...
freact = ({_, $, i18n}, BasicBackboneApplication, internationalize, AbstractModelComponent, validation, validators, AppRouter, resources)->
    ...
    class Application extends BasicBackboneApplication
        ...
        init: ->
            ...
            @addInitializer (options)->
                ...
                # we will also use the 'default' title engine to be consistent with page title
                titleEngine = router.title('default')
                        ...
                        ###
                            ...
                            'spClick' is a custom defined attribute that will execute the given expression on click.
                                ...
                                For example if you want to do onClick doThat(withThis, andThat),
                                    you will have to create doThat = (evt)->
                                    then put on you node data-withThis={withThis}
                                    and in doThat get the parameter using evt.currentTarget.getAttribute.
                                It works, but
                                    - attributes are limited to strings. Anything else is not allowed
                                    - the more variables you have, the more attributes you have to create
                                With 'spClick' you know that click is supposed to set language lng on first read
                        ###
                                ...
                                <ul className="menu nav nav-pills">
                                    <li spRepeat="(entry, index) in menuCollection" key={index}>
                                        <a href={ routerEngine.getUrl(entry) }>
                                            { i18n.t(titleEngine.getUrl(entry)) }
                                        </a>
                                    </li>
                                </ul>
                            </div>

                ...
                handlers: [
                    {
                        ...
                        name: 'controller' # to identify the handler
                        type: 'controller' # to consider loaded files as controllers
                        route: '{module}/controllers/{controller}'
                        camel: true # transform some-controller-name to SomeControllerName
                        suffix: 'Controller'
                    }
                    {
                        # if it exists, render the view defined in app/node_modules/{module}/views/{controller}/{Action}View
                        name: 'view' # to identify the handler
                        type: 'view' # to consider loaded files as views
                        route: '{module}/views/{controller}/{action}'
                        camel: true # transform some-view-name to SomeViewName
                        suffix: 'View'
                    }
                    {
                        # if it exists, display the template file app/node_modules/{ module }/templates/{ controller }/{ action }
                        type: 'template' # to identify the handler
